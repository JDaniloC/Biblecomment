    ...
    debounceRef.current = setTimeout(async () => {
      ...
      try {
        ...
        const merged: ResultRow[] = [
          ...
          ...data.comments.map((c): ResultRow => ({ kind: "comment", data: c })),
        ];
        setResults(merged);
      } catch {
        setResults([]);
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-slate-950">
      ...
      <main id="main-content" className="max-w-3xl mx-auto px-4 py-6">
        ...
        <p className="text-sm text-gray-500 dark:text-slate-400 mb-5">
          Procure por trecho do versículo, texto de um comentário, ou referência (ex.: <span className="font-medium">Gn 1:1</span> ou <span className="font-medium">Gênesis 1</span>).
        </p>
          ...
          <ul className="space-y-3">
            {results.map((row) => (
              <li
                ...
                onClick={() => goTo(row)}
                onKeyDown={(e) => {
                  if (e.key === "Enter" || e.key === " ") { e.preventDefault(); goTo(row); }
                }}
              ...
              >
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-xs text-blue-600 dark:text-brand font-medium">
                    {row.kind === "verse" ? row.data.reference : row.data.bookReference}
                  </span>
                </div>
                <p className="text-sm text-gray-700 dark:text-slate-200 line-clamp-3">{row.data.text}</p>
                {row.kind === "comment" && (
                  <div className="text-xs text-gray-400 dark:text-slate-500 mt-1">por {row.data.username}</div>
                )}
              </li>
            ))}
          </ul>
