...
function setup(
	...
	return {
		svc: new PushNotificationService(repo, sender),
		send,
		deleteByEndpoints,
	};
}
...
describe("PushNotificationService", () => {
	it("sends the serialized payload to every subscription of the user", async () => {
		const { svc, send } = setup([sub("e1"), sub("e2")], async () => ({
			ok: true,
		}));
	...
	it("prunes only 404/410 endpoints, keeps others", async () => {
		...
		const arg = (deleteByEndpoints as ReturnType<typeof vi.fn>).mock
			.calls[0][0];
		expect([...arg].sort()).toEqual(["gone404", "gone410"]);
	});

	it("no subscriptions → no send, no prune", async () => {
		const { svc, send, deleteByEndpoints } = setup([], async () => ({
			ok: true,
		}));
		await svc.sendToUser("alice", payload);
