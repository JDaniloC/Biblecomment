...
export function ProfileProvider ({ children }) {
        ...
        try {
            await axios.get("users/favorites", {
                ...
                if (response.data.favorites !== undefined) {
                    ...
                    let length = Math.ceil(newResult.length / 5);
                    if (newFavorites.length === 50) { length += 1 } 
                    else if (page !== 1) { page -= 1 }
    ...
    return (
        <ProfileContext.Provider value = {{
            email, name, stateName, belief, 
            chaptersCount, commentsCount,
            currentFPage, currentCPage, 
            totalCPages, totalFPages, 
            commentaries, favorites, 
            perfilClass, formClass, 
            commented, booksCount,
            loadUserInfos, getComments,
            setCommentaries, setBelief,
            setPerfilClass, setFavorites,
            setFormClass, setCurrentCPage, 
            setCurrentFPage, getFavorites,            
            setStateName, addNewComment
        }}>
            {children}
