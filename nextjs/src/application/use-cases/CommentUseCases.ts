...
export class GetUserFavoritesUseCase {
	...
	async execute(
		...
		const ids = await this.likeRepo.findCommentIdsLikedBy(
			userId,
			page,
			pageSize,
		);
...
export class CreateCommentUseCase {
	...
	async execute(input: CreateCommentInput): Promise<Comment> {
		...
		const bookRef =
			verse.reference ??
			`${verse.abbrev} ${verse.chapter}:${verse.verseNumber}`;
...
export class DeleteCommentUseCase {
	...
	async execute(
		...
		const comment = await this.commentRepo.findById(id);
		if (!comment) throw new Error("Comment not found");
		if (!isModerator && comment.username !== username)
...
export class ListReportedCommentsUseCase {
	 ...
	 */
	async execute(page: number, pageSize: number): Promise<Comment[]> {
		const aggregates = await this.reportRepo.findReportedCommentIds(
			page,
			pageSize,
		);
...
export class ClearReportsUseCase {
	...
	async execute(
		commentId: string,
	): Promise<{ commentId: string; cleared: number }> {
