...
describe("@mentions", () => {
	...
	describe("In comments", () => {
		...
		it("self-mention @alice by alice creates no notification", () => {
			...
			cy.request("/api/notifications").then((res) => {
				expect(res.body.unread, "self-mention must not notify the actor").to.eq(
					0,
				);
			});
		});
		...
		it("email-shaped strings do NOT trigger mentions", () => {
			...
			getVerseId("gn", 1, 1).then((verseId) => {
				cy.request({
					...
					url: `/api/comments/${verseId}`,
					body: {
						text: "Me escreva em foo@bar.com ou bob@example.com",
						tags: [],
					},
				});
			});
			...
			cy.task<{ id: string }>("db:seedComment", {
				...
				.then((createRes) => {
					...
					cy.request("/api/notifications").then((res) => {
						expect(
							res.body.unread,
							"mod should have 1 mention notification",
						).to.eq(1);
						const note = res.body.items.find(
							(n: { type: string }) => n.type === "answer_mention",
						);
						assert.exists(note, "answer_mention notification missing");
					...
					cy.loginAs(users.alice.email, users.alice.password);
					cy.request("/api/notifications").then((res) => {
						const types = (res.body.items as Array<{ type: string }>).map(
							(n) => n.type,
						);
