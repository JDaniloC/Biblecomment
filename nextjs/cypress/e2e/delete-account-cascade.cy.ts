        ...
        cy.request({
          ...
          cy.request({
            ...
            cy.request("GET", `/api/discussion/gn/${aliceDiscussionId}`).then((discRes) => {
              ...
              const bobAnswer = d.answers.find((a: { name: string }) => a.name === "bob");
            ...
            cy.task<{ exists: boolean }>("db:findUser", users.alice.email).then((u) => {
              expect(u.exists, "alice's user document is hard-deleted").to.be.false;
            });
          });
        });
      });
  ...
  it("when alice answered someone else's discussion, only her embedded answer is anonymized", () => {
    ...
    cy.request({
      ...
      body: {
        ...
        verseText: "A terra era sem forma e vazia; havia trevas sobre a face do abismo.",
        commentText: "",
      ...
      cy.request("GET", `/api/discussion/gn/${bobDiscussionId}`).then((postRes) => {
        const d = postRes.body;
        expect(d.username, "bob's discussion ownership untouched").to.eq("bob");
        const aliceAnswer = d.answers.find(
          (a: { text: string }) => a.text === "Reflexão da alice sobre o tohu va-bohu.",
        );
        expect(aliceAnswer, "alice's answer text preserved").to.exist;
        expect(aliceAnswer.name, "alice's name in answer is anonymized").to.eq(ANON);
      });
    });
  });
});
