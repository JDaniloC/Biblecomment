...
describe("Badges", () => {
	...
	describe("mark chapter as read", () => {
		...
		it("first mark unlocks first-read + reader-bronze countdown begins", () => {
			...
			cy.task<number>("db:countChapterReads", users.alice.email).should(
				"eq",
				1,
			);
			cy.task<string[]>("db:getUserBadges", users.alice.email).should(
				"include",
				"first-read",
			);
		});
	...
	describe("Conquistas tab", () => {
		it("renders all 30 catalog badges, with a summary count", () => {
			...
			cy.get('[data-testid="badges-summary"]').should(
				"contain.text",
				"0 de 34",
			);
		...
		it("flips a card to earned after the user marks a chapter", () => {
			...
			cy.get('[data-testid="badge-card-first-read"]').should(
				"have.attr",
				"data-earned",
				"true",
			);
