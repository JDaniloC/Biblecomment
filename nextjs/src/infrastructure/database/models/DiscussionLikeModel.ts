...
const DiscussionLikeSchema = new Schema<IDiscussionLikeDocument>(
  {
    ...
    targetType: { type: String, required: true, enum: ["discussion", "answer"] },
...
DiscussionLikeSchema.index({ userId: 1, targetType: 1, targetId: 1 }, { unique: true });
...
export const DiscussionLikeModel: Model<IDiscussionLikeDocument> =
  ...
  mongoose.model<IDiscussionLikeDocument>("DiscussionLike", DiscussionLikeSchema);
