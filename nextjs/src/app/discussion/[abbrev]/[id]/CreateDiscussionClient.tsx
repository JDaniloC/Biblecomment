...
export default function CreateDiscussionClient({ book, user, sourceComment }: Props) {
  ...
  function captureSelection() {
    ...
    if (!node || !sel || sel.rangeCount === 0 || sel.isCollapsed) {
      handleNotification("error", "Selecione um trecho do comentário primeiro.");
      return;
    }
  ...
  const hasHighlight = typeof quoteStart === "number" && typeof quoteEnd === "number";
      ...
      <main id="main-content" className="max-w-3xl mx-auto px-4 py-6 space-y-5">
        ...
        <section className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-4">
          <div className="flex items-center justify-between gap-2 mb-2">
            <p className="text-xs font-medium text-brand">{sourceComment.reference}</p>
            <Link
