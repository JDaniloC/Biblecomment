...
export class MongoDiscussionLikeRepository implements IDiscussionLikeRepository {
  async like(userId: string, targetType: DiscussionLikeTarget, targetId: string): Promise<boolean> {
  ...
  async unlike(userId: string, targetType: DiscussionLikeTarget, targetId: string): Promise<void> {
  ...
  async hasLiked(userId: string, targetType: DiscussionLikeTarget, targetId: string): Promise<boolean> {
    if (!mongoose.Types.ObjectId.isValid(targetId)) return false;
    await connectToDatabase();
  ...
  async countByTargets(
    ...
    const rows = await DiscussionLikeModel.aggregate<{ _id: mongoose.Types.ObjectId; total: number }>([
  ...
  async deleteByTarget(targetType: DiscussionLikeTarget, targetId: string): Promise<number> {
